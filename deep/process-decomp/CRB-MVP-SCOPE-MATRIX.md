...
**Customer intelligence surface (Module C)**
| L3 ID | Process | Why Category A |
|-------|---------|---------------|
| C.2.1–C.2.4 | Customer risk scoring and behavioral pattern detection | Canary intelligence — no Counterpoint equivalent |
| C.6.1–C.6.4 | Investigator surface (lookup, history, risk score, cross-module context) | How the investigator acts on a Q alert. Canary's operator UX |
...
| C | M.4.1–M.4.5 | B2B detection rules Q-M-01 through Q-M-05 | Routes through Q's Chirp pipeline. B2B LP intelligence |
| L | L.3.3 | Employee productivity signal to Q (LP context) | Connects labor data to LP investigation. Canary intelligence |
| W | E.1.1–E.5.6 | Cross-domain exception detection, aggregation, case management, remediation, MCP tools | The v3 capstone. W is Canary's immune system — generalizes Q across all domains |
...
| C.1 | C.1.1–C.1.5 | Customer profile sync (read-through, no PII at rest) | Customer master |
| C.3–C.5 | C.3.1–C.5.4 | Customer search, purchase history, privacy/consent | Customer transaction and AR data |
...
| O.6–O.7 | O.6.1–O.6.14, O.7.1–O.7.7 | Receiving lifecycle (RECVR documents), RTV lifecycle | Inbound goods receipt, vendor returns |
...
| M.1–M.3, M.5 — B2B account management, credit/invoicing, contract pricing, AR reporting | 21 | Counterpoint customer accounts + QuickBooks AR + accounting software | Canary detects B2B LP patterns (Category A, via Q). Account and AR management stays in existing systems. Canary can expose B2B analytics via API |
...
| O.4 — PO generation and write-back | 5 | Counterpoint manages PO documents. Buyer executes in Counterpoint | O.4.1–O.4.3: Canary can surface suggested orders as an analytics output. O.4.4 (write-back to Counterpoint): build only if buyer workflow clearly benefits; default posture is export-to-buyer's-workflow, not Canary-owns-the-PO |

### The Write-Back Question

Canary reads everything. The only identified write-back in the corpus is O.4.4 (`POST /Document`, DOC_TYP=PO). The correct decision framework:
...
| C — Customer | ~8 (risk scoring, investigator surface) | ~24 (sync, search, history, privacy) | 0 | Read-through + LP investigator context |
...
| J — Orders | 0 | ~14 (receiving, RTV read-through) | ~33 (forecasting, OTB, PO engine) | Receiving data + export API for planning tools |
...
| W — Execution | ~14 (all — v3 capstone) | 0 | 0 | Cross-domain LP immune system |
| **TOTAL** | **~125** | **~162** | **~120** | |
...
**v2+ — Selective workflow absorption:** Based on customer demand, absorb specific Category C workflows where Canary can demonstrably beat the existing solution. The candidate list: O.4 PO approval workflow (once receiving volume justifies it), S.6 range analytics (if no planning tool is connected), P.6 promotion calendar (if it directly improves Q accuracy).
