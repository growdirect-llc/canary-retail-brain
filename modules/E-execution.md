...
E is one of the [[spine-13-prefix#v3-full-spine-s-p-l-w|v3 full-spine expansion]] modules and the capstone of the spine. It is the retail operating system's immune system: every module publishes its invariant violations to E; E correlates them, escalates, and ensures visibility. Without E, violations go undetected and compound silently.
...
- ARTS POSLog carries multi-domain context (transaction, customer, device, employee, location) — all ARTS dimensions feed into E exception detection
...
- E **reads** all spine modules' published events and exception signals
- E **evaluates** each domain's rule catalog against observed events
- E **publishes** exception alerts (unconfirmed violations)
- E **escalates** high-confidence violations to case creation
- E **aggregates** exceptions across domains (e.g., is this customer involved in both inventory shrink and return fraud? Is this employee involved in both cash variance and missing inventory?)
...
- **S (Space):** over-capacity alert on planogram
...
- **S (Space)** — capacity violations, planogram conflict alerts
- **P (Pricing & Promotion)** — price-discrepancy flags, markdown policy violations
- **L (Labor)** — schedule anomalies, time-entry variance, productivity outliers
- **D (Distribution)** — receipt discrepancies, transfer variances, RTV anomalies
- **J (Orders)** — forecast-vs-actual misses, order anomalies
- **F (Finance)** — invoice variances, GL reconciliation mismatches

**Downstream consumers** (E's outputs):

- **Investigator agents** — via E's case MCP tools
- **All spine modules** — E may request remediation (D posts adjustment, T issues refund, S revalidates planogram, etc.)
...
- **v3.2** — Pattern learning (E learns common exception signatures and predicts escalation likelihood). Investigator-skill routing (assign cases to investigators with relevant expertise).
- **v3.3** — Predictive anomaly detection (E predicts exceptions before they occur, based on historical patterns and real-time signals).
...
- [[../platform/spine-13-prefix|The Canary Retail Spine — 13 Modules (E is the capstone)]]
